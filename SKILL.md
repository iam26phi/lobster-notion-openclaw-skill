...
- 300 字摘要 + 條列 3–5 則重點
- 台灣繁體中文
- **用第三人稱**：開頭用使用者名字，例如「26 今天做了 X」「小明今天新增了…」。**不用「你」**，因為這份草稿會貼到共學團共用 Notion，讀者需要看得出主體是誰。
- 使用者名字由 `scripts/resolve-user-name.sh` 在第一次執行時從 `workspace/USER.md` 的「What to call them」或「Name」欄位抓出，寫入 `config.user_name`，之後直接沿用。
- 不列出每個檔案、不貼 diff
